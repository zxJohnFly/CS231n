...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    out, cache_layer12 = affine_relu_forward(X, self.params['W1'], self.params['b1'])
    scores, cache_layer3 = affine_forward(out, self.params['W2'], self.params['b2'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    W1 = self.params['W1']
    W2 = self.params['W2']
    loss += 0.50*self.reg*(np.sum(W1*W1) + np.sum(W2*W2))

    dx, dW2, db2 = affine_backward(dx, cache_layer3)
    dx, dW1, db1 = affine_relu_backward(dx, cache_layer12)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for layer in xrange(self.num_layers):
      self.params[layer*2]     = weight_scale * np.random.randn(hidden_dims[layer], hidden_dims[layer + 1])
      self.params[layer*2 + 1] = np.zeros(hidden_dims[layer + 1])
